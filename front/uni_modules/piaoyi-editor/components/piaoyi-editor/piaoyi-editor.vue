			...
			<view class="page-body">
				<view class='wrapper'>
					...
					<view class='toolbar' @tap="format">
						...
						<view class="iconfont icon-shipin" @tap="insertMedium"></view>
					</view>
		...
		methods: {
			...
			insertImage() {
				...
				this.$api.upload(function(res) {
					const imageUrl = 'http://localhost:8088/tangfen/upload/' + res.file; // 假设上传的图片路径可以从响应中获取
					// 插入图片到富文本编辑器
			...
			insertMedium(){
				...
				this.$api.uploadMedia(function(res) {
					const videoUrl = 'http://localhost:8088/tangfen/upload/' + res.file; // 假设上传的图片路径可以从响应中获取
					// 插入图片到富文本编辑器
					that.editorCtx.insertMedium({
						src: videoUrl,
						alt: '视频',
						success: function() {}
					});
...
<style>
	...
	.editor-wrapper {
		height: calc(100vh - var(--window-top) - var(--status-bar-height) - 90px);
		background: #fff;
	}
	...
	.editor {
		...
		line-break: anywhere;
		height: calc(100vh - var(--window-top) - var(--status-bar-height) - 100px);
	}
</style>