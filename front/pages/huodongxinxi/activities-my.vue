	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="list">
			<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
				<image src="/static/exam-index/activity.png" mode="widthFix" class="listmpic mt10"></image>
				<view class="listmr">
	
					<view style="display: flex;">
						<view class="col3 f30 elip mb15" style="flex: 1;">{{item.huodongmingcheng}}</view>
						<view  style="felx:1;" >通过</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			console.log(params)
			    // 发送请求获取用户发布的活动信息
			    let res = await this.$api.page(`huodongxinxi`, params);
			...
			// 详情
			onDetailTap(id) {
				this.$utils.jump(`../huodongxinxi/detail?id=${id}`)
			},
			onUpdateTap(id) {
				if (id) {
					this.$utils.jump(`../huodongxinxi/add-or-update?id=${id}`)
