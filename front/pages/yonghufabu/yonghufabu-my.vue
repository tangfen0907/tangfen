	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="list">
			<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
				 <view ><image
				 	mode="aspectFill" class="listmpic "  :src="baseUrl+item.tupian"></image></view>
				
				<view class="listmr">
					
					<view style="display: flex;">
						<view class="col3 f30 elip mb15" style="flex: 1;">{{item.biaoti}}</view>
						<view  style="felx:1;" >通过
						</view>
					...
					<view class="flex flex-align-center mt20">
						<view class="colb f25">
							...
							<button style="margin-left: 20upx;" @tap="onUpdateTap(item.id)"
								class="cu-btn bg-blue shadow">修改</button>
							<button style="margin-left: 20upx;" @tap="onDeleteTap(item.id)"
								class="cu-btn bg-red shadow">删除</button>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
			    ...
			    // 发送请求获取用户发布的攻略信息
			    let res = await this.$api.page('yonghufabu', params);
			...
			onDetailTap(id) {
				console.log(id);
				this.$utils.jump(`../yonghufabu/detail?id=${id}`)
			},
			onUpdateTap(id) {
				if (id) {
					this.$utils.jump(`../yonghufabu/add-or-update?id=${id}`)
				} else {
