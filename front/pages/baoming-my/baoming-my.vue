			...
			<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
				...
				<view class="listmr">
					<view style="display: flex;">
						<view class="col3 f30 elip mb15" style="flex: 1;">{{item.activityname}}</view>
						<view  style="felx:1;" :style="{ color: getStatusColor(item.status) }">{{item.status}}
						</view>

					</view>
					<view v-if="item.addtime" class="colb f24 lh35">报名时间{{item.addtime}}</view>
					<view class="flex flex-align-center mt20">
						<view class="colb f25">
							
							<button @tap="onDetailTap(item.activityid)" class="cu-btn bg-green shadow">查看</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				// 发送请求获取用户发布的活动信息
				let res = await this.$api.page(`baomingxinxi`, params);
