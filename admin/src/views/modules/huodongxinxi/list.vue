<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item class="select" label="活动地区" prop="huodongdiqu">
            <el-select @change="huodongdiquChange" clearable v-model="searchForm.huodongdiqu" placeholder="请选择活动地区">
              <el-option
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '活动名称' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search"
                      v-model="searchForm.huodongmingcheng" placeholder="活动名称" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
                      v-model="searchForm.huodongmingcheng" placeholder="活动名称" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.huodongmingcheng" placeholder="活动名称"
                      clearable></el-input>
          </el-form-item>
          <el-form-item class="select" label="活动类型" prop="huodongleixing">
            <el-select @change="huodongleixingChange" clearable v-model="searchForm.huodongleixing"
                       placeholder="请选择活动类型">
              <el-option
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '活动地点' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search"
                      v-model="searchForm.huodongdidian" placeholder="活动地点" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
                      v-model="searchForm.huodongdidian" placeholder="活动地点" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.huodongdidian" placeholder="活动地点"
                      clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           ...
                           label="行程安排">
            <template slot-scope="scope">
              {{ scope.row.xingchenganpai }}
            </template>
          </el-table-column>

          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="feiyongshuoming"
                           :header-align="contents.tableAlign"
                           label="行程安排">
            <template slot-scope="scope">
              {{ scope.row.feiyongshuoming }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="zhuangbeiyaoqiu"
                           :header-align="contents.tableAlign"
                           label="精选活动">
            <template slot-scope="scope">
              {{ scope.row.zhuangbeiyaoqiu }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           prop="zhuangbeiyaoqiu"
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              <el-button
                  v-if="isAuth('huodongxinxi','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">
                {{ contents.tableBtnFont == 1 ? '详情' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('huodongxinxi','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('huodongxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1 ? '详情' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('huodongxinxi','报名活动') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="success" icon="el-icon-tickets" size="mini"
                  @click="dingdanxinxiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','您已提交过报名！')">
                {{ contents.tableBtnFont == 1 ? '报名活动' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('huodongxinxi','报名活动') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  type="success" size="mini"
                  @click="dingdanxinxiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','您已提交过报名！')">
                {{ contents.tableBtnFont == 1 ? '报名活动' : '' }}<i class="el-icon-tickets el-icon--right"/></el-button>
              <el-button v-if="isAuth('huodongxinxi','报名活动') && contents.tableBtnIcon == 0" type="success" size="mini"
                         @click="dingdanxinxiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','您已提交过报名！')">
                {{ contents.tableBtnFont == 1 ? '报名活动' : '' }}
              </el-button>
              <el-button
                  v-if=" isAuth('huodongxinxi','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '修改' : '' }}
              </el-button>
              <el-button
                  v-if=" isAuth('huodongxinxi','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if=" isAuth('huodongxinxi','修改') && contents.tableBtnIcon == 0" type="primary" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? '修改' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('huodongxinxi','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '删除' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('huodongxinxi','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('huodongxinxi','删除') && contents.tableBtnIcon == 0" type="danger" size="mini"
    ...
    <dingdanxinxi-cross-add-or-update v-if="dingdanxinxiCrossAddOrUpdateFlag" :parent="this"
                                      ref="dingdanxinxiCrossaddOrUpdate"></dingdanxinxi-cross-add-or-update>
